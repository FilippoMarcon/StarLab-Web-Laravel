...
<div class="p-6 bg-slate-900/50 border border-slate-800 rounded-2xl lg:sticky lg:top-6">
    <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-1.5 text-xs">
            <span class="w-2 h-2 rounded-full {{ $online ? 'bg-emerald-500' : 'bg-slate-600' }}"></span>
            <span class="{{ $online ? 'text-emerald-400' : 'text-slate-500' }} font-bold">{{ $online ? 'Online' : 'Offline' }}</span>
            @if (!$online && $lastSeen)
            <span class="text-slate-600">&middot; {{ $lastSeen->diffForHumans() }}</span>
            @endif
        </div>
    </div>
...
(function() {
    ...
    {{-- Init seen IDs from server-rendered messages --}}
