...
<form method="POST" action="{{ $project ? route('admin.projects.update', $project) : route('admin.projects.store') }}" class="max-w-2xl space-y-6">
    @csrf
    ...
    <div>
        <label class="block text-sm font-bold text-slate-300 mb-2">URL Copertina</label>
        <input type="url" name="cover_image" value="{{ old('cover_image', $project->cover_image ?? '') }}"
            class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all">
