...
<body class="bg-zinc-950 text-zinc-100 min-h-screen">
    ...
    function renderNotifications(data) {
        ...
        data.items.forEach(function(item) {
            var dot = item.type === 'new_quote' ? 'bg-amber-500' : (item.type === 'pending_reply' ? 'bg-sky-500' : 'bg-emerald-500');
    ...
    function checkNotifications() {
        ...
        xhr.open('GET', '{{ route('api.admin.notifications') }}', true);
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                if (data.total > 0) {
                    ...
                    if (data.total > prevTotal && prevTotal > 0) {
                        ...
                        showToast(first.text, first.type === 'new_quote' ? 'info' : 'info');
                    }
                } else {
